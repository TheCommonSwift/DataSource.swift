...
class TableViewDataSource<CellType: UITableViewCell>: NSObject, UITableViewDataSource {
	...
	init(tableView tv: UITableView, dataProvider dp: DataProvider,
        delegate d: TableViewDataSourceDelegate,
        cellConfigurationHandler h: (CellType, AnyObject) -> ()) {
		tableView = tv
		dataProvider = dp
		delegate = d
    ...
    init(tableView tv: UITableView, dataProvider dp: DataProvider, cellReuseIdentifier id: String,
        cellConfigurationHandler h: (CellType, AnyObject) -> ()) {
        tableView = tv
        dataProvider = dp
    ...
    private func reuseIdentifierForObject(object: AnyObject) -> String {
        ...
        fatalError("Either cellReuseIdentifier must be set or cellIdentifierForObject must " +
	...
	func tableView(tableView: UITableView,
		...
		guard let cell = tableView.dequeueReusableCellWithIdentifier(id, forIndexPath: indexPath)
            as? CellType else { fatalError("The dequeued cell has the wrong type") }
